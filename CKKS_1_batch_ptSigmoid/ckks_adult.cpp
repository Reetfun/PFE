...
vector<Ciphertext<DCRTPoly>> PolynomialSigmoidChebyshev(vector<Ciphertext<DCRTPoly>> ct, CryptoContext<DCRTPoly> cc, int degree) {
    ...
    for (int i = 0; i < int(ct.size()); i++) {
        ct_sig.push_back(cc->EvalChebyshevFunction([](double x) -> double { return 1 / (1 + exp(-x)); }, ct[i], -6, 6, degree));
    }
    return ct_sig;
}

vector<vector<double>> realSigmoid(vector<vector<double>> verif) {
...
vector<vector<double>> ptSigmoid(vector<Plaintext> data) {
    vector<vector<double>> res;
    for (int i = 0; i < int(data.size()); i++) {
        vector<double> row;
        for (int j = 0; j < int(data[0]->GetRealPackedValue().size()); j++) {
            double ct = 1 / (1 + exp(-data[i]->GetRealPackedValue()[j]));
            row.push_back(ct);
        }
        res.push_back(row);
    }
...
void evaluateStats(vector<vector<double>> results_verif, vector<vector<double>> results, vector<int> labels, string dir_path, int batch_size, int index_loop, int mult_depth, int polynomial_degree, int scaling_modulus, CryptoContext<DCRTPoly> cc, vector<double> time) {

    for (int i = 0; i < batch_size; i++) {
        vector<double> stats;
        stats.push_back(abs(results_verif[0][i] - results[0][i]));
        stats.push_back(abs(labels[i + index_loop * batch_size] - results[0][i]));
        stats.push_back(isPredictionCorrect(labels[i + index_loop * batch_size], results[0][i]));
        stats.push_back(results_verif[0][i]);
        stats.push_back(results[0][i]);
    ...
    ofstream file(dir_path + "/" + "time.csv", ios::app);
    if (!file.is_open()) {
        cerr << "Error !" << endl;
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    // ==================================================================================
    string dir_path = initDirectory("../CKKS_1_batch_ptSigmoid/results", polynomial_degree, batch_size, i);
    ...
    // ==================================================================================
    cout << "\n{...} Evaluation of the activation function of the third layer in clear" << endl;
    TIC(t);
    vector<Plaintext> pt_layer_3 = decryptVector(ct_layer_3, cc, keys.secretKey, batch_size);
    vector<vector<double>> layer_3_act = ptSigmoid(pt_layer_3);
    processing_time = TOC(t);
    time.push_back(processing_time);
    ...
    vector<vector<double>> layer_3_act_verif = realSigmoid(layer_3_verif);
    ...
    evaluateStats(layer_3_act_verif, layer_3_act, labels, dir_path, batch_size, i, mult_depth, polynomial_degree, scaling_modulus_bits, cc, time);
