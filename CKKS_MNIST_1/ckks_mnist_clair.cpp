...
vector<int> getLabels(int batch_size) {
    ...
    ifstream inputFile("../CKKS_MNIST_1/MNIST-28x28-test.txt");
...
vector<vector<double>> getInputs(int batch_size, double normalizationFactor, double scale) {
    ...
    ifstream file("../BFV_MNIST_2/MNIST-28x28-test.txt");
    ...
    while (!file.eof() && int(inputData.size()) < batch_size)
    {
        ...
        for (int i = 0; i < 28*28; i++) {
            if (i == features[0].first) {
                row.push_back(features[0].second * scale);
...
vector<double> multVectorAndInt(vector<double> data, double weight) {
    ...
    for (int i = 0; i < int(data.size()); i++) {
        result.push_back(data[i] * weight);
    }
    return result;
} 
                ...
                for (int x = 0; x < kernel_dim; x++) { // Pour chaque ligne du filtre
                    for (int y = 0; y < kernel_dim; y++) { // Pour chaque colonne du filtre
                        ...
                        if (row < dim && col < dim) { // Padding en bas et à droite !
                            vector<double> mult_result = multVectorAndInt(inputs[row*dim + col], weight);
...
void saveResults(string path, vector<vector<double>> decrypted_results, vector<int> labels, int batch_size, int nb_true, int nb_false, int nb_total, vector<double> time_results) {
    ofstream file;
    ...
    if (file3.is_open()) {
        file3 << "nb_true,nb_false,nb_total,precision" << endl;
        file3 << nb_true << "," << nb_false << "," << nb_total << "," << double(nb_true)/double(nb_total) << endl;
        file3.close();
    } else {
        cerr << "Error opening the file." << endl;
    }
...
int main(int argc, char *argv[]) {
    ...
    // // ==================================================================================
    string dir_path = initDirectory("../CKKS_MNIST_1/results_pt", batch_size);
    ...
    // vector<vector<double>> input = transposeDataReal(getInputsReal(batch_size, normalization_factor));
    // Scale : 16
    ...
    //         cout << "index: " << i << " | value: " << input[i][j] << endl;
    //     }
    //     cout << endl;
    // }
    ...
    int padding = 1; // Padding à appliquer aux bords de l'image (nbre de rangée de 0 à ajouter) en bas et à droite  
    int dim = 28; // Dimension de l'image
    // int output_dim = (dim - kernel_dim + padding) / stride + 1; // = 13 

    TIC(t);
    vector<vector<double>> conv_results = ptConvLayer(input, weights_0, bias_0, kernel_dim, stride, dim, conv_scale, padding, batch_size);
    processing_time = TOC(t);
    ...
    for (int i = 0; i < 13; i++) {
        for (int j = 0; j < 13; j++) {
            // cout << "(" << i << "-" << j << ")-" << conv_results[i*13 + j][1] << " ";
            cout << conv_results[i*13 + j][1] << " ";
            // cout << conv_results[13*13 + i*13 + j][1] << " ";
        }
        cout << endl;
    }
    ...
    processing_time = TOC(t);
    time_results.push_back(processing_time);
    cout << "=> Square activation done [" << processing_time << " ms]" << endl;
    ...
    cout << "\n{...} FCLayer 2" << endl;
    TIC(t);
    vector<vector<double>> fc_results_2 = ptFCLayer(activation_results_1, weights_1_flatten, bias_1, fc_scale, batch_size);
    processing_time = TOC(t);
    time_results.push_back(processing_time);
    cout << "=> Fully connected layer done [" << processing_time << " ms]" << endl;
    ...
    cout << "\n{...} FCLayer 3" << endl;
    TIC(t);
    vector<vector<double>> fc_results_3 = ptFCLayer(activation_results_2, weights_2_flatten, bias_2, output_scale, batch_size);
    processing_time = TOC(t);
    time_results.push_back(processing_time);
    cout << "=> Fully connected layer done [" << processing_time << " ms]" << endl;
    ...
    for (int i = 0; i < batch_size; i++) {
        double max = fc_results_3[0][i];
        int index_max = 0;
        for (int j = 1; j < 10; j++) {
            if (fc_results_3[j][i]> max) {
                max = fc_results_3[j][i];
                index_max = j;
            }
        }
    ...
    // Save results
    // ==================================================================================
    saveResults(dir_path, fc_results_3, labels, batch_size, nb_true, nb_false, nb_total, time_results);
