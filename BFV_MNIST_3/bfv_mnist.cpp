...
CryptoContext<DCRTPoly> initSystem(int batch_size, int mult_depth, int64_t pt_modulus) {
    CCParams<CryptoContextBFVRNS> parametersBFV;
    parametersBFV.SetBatchSize(batch_size);
    parametersBFV.SetMultiplicativeDepth(mult_depth);
    parametersBFV.SetPlaintextModulus(pt_modulus); 
    parametersBFV.SetMaxRelinSkDeg(3);

    CryptoContext<DCRTPoly> cc = GenCryptoContext(parametersBFV);  
    cc->Enable(PKE);
    cc->Enable(KEYSWITCH);
...
vector<Ciphertext<DCRTPoly>> encryptBatch(vector<vector<int64_t>> data, CryptoContext<DCRTPoly> cc, PublicKey<DCRTPoly> pk) {
    vector<Ciphertext<DCRTPoly>> encrypted_data;
    for (int i = 0; i < int(data.size()); i++) {
        Plaintext pt = cc->MakePackedPlaintext(data[i]);
...
vector<vector<int64_t>> getInputs(int batch_size, double normalizationFactor, double scale) {
    ...
    while (!file.eof() && int(inputData.size()) < batch_size)
    {
        ...
        for (int i = 0; i < 28*28; i++) {
            if (i == features[0].first) {
                row.push_back(int64_t(round(features[0].second * scale)));
...
vector<vector<int64_t>> transposeData(vector<vector<int64_t>> data) {
   vector<vector<int64_t>> transposed_data;
...
vector<int64_t> fillVector(double weight, int scale, int batch_size) {
    vector<int64_t> weight_vector;
    for (int i = 0; i < batch_size; i++) {
        weight_vector.push_back(int64_t(round(weight * scale)));
    }
...
void encodeParams(vector<double> data, vector<Plaintext> &dest, CryptoContext<DCRTPoly> cc, int scale, int batch_size) {
    ...
    for (int i = 0; i < int(data.size()); i++) {
        try {
            dest.push_back(cc->MakePackedPlaintext(fillVector(data[i], scale, batch_size)));
        } catch (const exception &e) {
            cerr << "Error in encodeParams ! " << i << e.what() << endl;
        }
    }
    std::cout << "e" << endl;
}
    ...
    for (int k = 0; k < int(weights.size()); k++) { // Pour chaque filtre
        ...
        for (int j = 0; j < output_dim; j++) { // Pour chaque ligne de l'image de sortie
            ...
            for (int i = 0; i < output_dim; i++) { // Pour chaque colonne de l'image de sortie
                ...
                Ciphertext<DCRTPoly> kernel_result = cc->Encrypt(pk, cc->MakePackedPlaintext(fillVector(0, conv_scale, batch_size)));
                    ...
                    for (int x = 0; x < kernel_dim; x++) { // Pour chaque ligne du filtre
                        ...
                        for (int y = 0; y < kernel_dim; y++) { // Pour chaque colonne du filtre
                            ...
                            Plaintext pt_weight = cc->MakePackedPlaintext(fillVector(weight, conv_scale, batch_size));
...
vector<Ciphertext<DCRTPoly>> FCLayer(vector<Ciphertext<DCRTPoly>> ct_inputs, vector<double> weights, vector<Plaintext> pt_bias_1, CryptoContext<DCRTPoly> cc, double fc_scale, PublicKey<DCRTPoly> pk, int batch_size) {
    ...
    for (int i = 0; i < int(weights.size()); i += int(ct_inputs.size())) { 
        Ciphertext<DCRTPoly> row = cc->EvalMult(ct_inputs[0], cc->MakePackedPlaintext(fillVector(weights[i], fc_scale, batch_size)));
        for (int x = 1; x < int(ct_inputs.size()); x++) { 
            row = cc->EvalAdd(row, cc->EvalMult(ct_inputs[x], cc->MakePackedPlaintext(fillVector(weights[i + x], fc_scale, batch_size))));
        }
...
void saveResultsClear(string path, vector<Plaintext> results, vector<int> labels, int batch_size, int nb_true, int nb_false, int nb_total, vector<double> time_results) {
    ofstream file;
    file.open(path + "/" + "final_result.csv");
    if (file.is_open()) {
        for (int i = 0; i < batch_size; i++) {
            file << i << "," << results[0]->GetPackedValue()[i] << "," << results[1]->GetPackedValue()[i] << "," << results[2]->GetPackedValue()[i] << "," << results[3]->GetPackedValue()[i] << "," << results[4]->GetPackedValue()[i] << "," << results[5]->GetPackedValue()[i] << "," << results[6]->GetPackedValue()[i] << "," << results[7]->GetPackedValue()[i] << "," << results[8]->GetPackedValue()[i] << "," << results[9]->GetPackedValue()[i] << "," << labels[i] << endl;
        }
        file.close();
    } else {
    ...
    if (file2.is_open()) {
        file2 << "nb_true,nb_false,nb_total,precision,time_results" << endl;
        file2 << nb_true << "," << nb_false << "," << nb_total << "," << double(nb_true)/double(nb_total) << "," << time_results << endl;
        file2.close();
    } else {
        cerr << "Error opening the file." << endl;
    }
...
int main(int argc, char *argv[]) {
    ...
    int input_scale = 2; // 16
    int conv_scale = 2; // 32
    int fc_scale = 32; // 32
    int output_scale = 16; // 32
    ...
    int64_t pt_modulus = 10000039937; // 
    ...
    TIC(t);
    CryptoContext<DCRTPoly> cc = initSystem(batch_size, mult_depth, pt_modulus);
    auto keys = cc->KeyGen();
    ...
    // // ==================================================================================
    string dir_path = initDirectory("../BFV_MNIST_3/results", batch_size);
    ...
    vector<vector<int64_t>> input = transposeData(getInputs(batch_size, normalization_factor, input_scale));
    ...
    vector<Plaintext> decrypted_results = decryptVector(fc_results_3, cc, keys.secretKey);

    for (int i = 0; i < batch_size; i++) {
        int64_t max = decrypted_results[0]->GetPackedValue()[i];
        int index_max = 0;
        for (int j = 1; j < 10; j++) {
            if (decrypted_results[j]->GetPackedValue()[i] > max) {
                max = decrypted_results[j]->GetPackedValue()[i];
                index_max = j;
            }
        }
    ...
    saveResultsClear(dir_path, decrypted_results, labels, batch_size, nb_true, nb_false, nb_total, time_results);
